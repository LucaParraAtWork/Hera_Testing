...
for Week 21/2027 (May 24-30).  Profiles and bay occupancy consistency
checks belong in a separate script.

Scenarios covered:
  SETUP_01  Import Week 21 baseline → slots become NEW
  ...
  SETUP_02  Re-import Week 21 (same CSV) → fresh NEW slots for bulk tests
...
SCENARIOS = [
    ...
    {"n":  1, "id": "SETUP_01", "group": "setup",
     "name": "Import Week 21 baseline (Virya)",
     "desc": "Import Virya_Nomination_Week21.csv → all Virya slots become NEW",
     "expect": "PASS"},

    # ── Grid view ─────────────────────────────────────────────────────────
    {"n":  2, "id": "UI_01", "group": "view",
     "name": "View nominations grid – count NEW slots",
     "desc": "Navigate to Week 21, count NEW slots, screenshot",
     "expect": "PASS"},
    ...
    {"n": 14, "id": "SETUP_02", "group": "setup",
     "name": "Re-import Week 21 (reset to NEW)",
...
WEEK21_CSV    = HERE / "Virya_Nomination_Week21.csv"
WEEK20_CSV    = HERE / "Virya_Nomination_Week20.csv"
...
def _wait_for_auth_settle(page, timeout_s=15):
    ...
    another navigation'. Poll until the URL leaves the auth callback."""
...
# --------------------------------------------------------------------------
# CSV helpers (generate week 21 from week 20 if missing)
# --------------------------------------------------------------------------
def _ensure_week21_csvs():
    if WEEK21_CSV.exists():
        return
    if not WEEK20_CSV.exists():
        raise FileNotFoundError(f"Base CSV not found: {WEEK20_CSV}")
    ...
    _shift(WEEK20_CSV, WEEK21_CSV)
    print(f"  Generated: {WEEK21_CSV.name}")
...
def main():
    ...
    print(f"  Target week   : {TARGET_WEEK}/{TARGET_YEAR}  (May 24-30 2027)")
            ...
            try:
                ...
                if   sid == "SETUP_01":
                    shot, auto, reason = _do_import(
                        page, nominations_url, WEEK21_CSV, shots_dir)
                elif sid == "SETUP_02":
                    shot, auto, reason = _do_import(
                        page, nominations_url, WEEK21_CSV, shots_dir)
                elif sid == "UI_01":
